...
@section('content')
    <div class="container">
        <x-adminlte-card title="Room Types" theme="dark" class="mt-4">
            <div class="row mb-3">
                <div class="col">
                    <a href="{{route('admin.room-types.create')}}" class="btn btn-success float-right">Add New</a>
                </div>
            </div>
    ...
    <script>
        $(function () {
            ...
            var table = $('.data-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('admin.room-types.index') }}",
                columns: [
                    {data: 'id', name: 'id'},
                    {data: 'name', name: 'name'},
